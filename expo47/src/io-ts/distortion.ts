...
export const DistortionFromData = Data.pipe(new T.Type<Distortion, Data, Data>(
    'DistortionFromData',
    (val): val is Distortion => val instanceof Distortion,
    (data, c) => T.success(new Distortion(data)),
    (dis) => dis.data,
))
export const DistortionFromID = T.string.pipe(new T.Type<Distortion, ID, ID>(
    'DistortionFromID',
    DistortionFromData.is,
    (id, c) => id in bySlug ? T.success(bySlug[id]) : T.failure("no such distortion-id", c),
    (dis) => dis.slug,
))
export const DistortionFromLegacy = Legacy.pipe(new T.Type<Distortion, Legacy, Legacy>(
...
export const DistortionFromAny = T.union([DistortionFromID, DistortionFromLegacy])
