...
function instance<A, O = A, I = unknown>(cls: new (input: I) => A, encode: (t: A) => O): T.Type<A, O, I> {
...
test('encode', () => {
    ...
    if (false) expect(TT.DateFromNumber.pipe(TT.NumberFromString).decode("12345")).toEqual(E.right(new Date(12345)))
    ...
    expect(Reporter.report(CFromNumber3.decode("3"))).toEqual(["Invalid value \"3\" supplied to : CFromNumber3"])
